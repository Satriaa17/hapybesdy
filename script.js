...
const layers = [
  { 
    name: 'Hello',
    story: 'Aku ketemu kamu pertama kali di Telegram — obrolan sederhana, tapi pelan-pelan jadi cerita yang berarti.',
    action: (cx,cy) => explodeAt(cx,cy,12)
  },
  { 
    name: 'Bloom',
    story: 'Kemarin Valentine. Walau sekarang kita sudah bukan apa-apa, aku tetap menghargai rasa yang pernah tumbuh.',
    action: (cx,cy) => { confettiBurst(cx,cy,24); explodeAt(cx,cy,18); }
  },
  { 
    name: 'Promise',
    story: 'Ini soal janji. Bukan buat mengulang, tapi buat menepati apa yang pernah aku ucapkan.',
    action: (cx,cy) => { playTune(); showToast('happy birthday'); }
  },
  { 
    name: 'Today',
    story: 'Selamat ulang tahun. Semoga kamu baik-baik saja, bahagia dengan jalan kamu sendiri.',
  ...
  async function tryAutoPlayBg() {
    ...
    try {
      ...
      isBgPlaying = true;
      updateMusicBtnUI();
      showToast('Musik diputar otomatis 🎵');
    } catch (err) {
      // audible autoplay blocked — prepare interaction-based unlock (no button required)
      isBgPlaying = false;
      updateMusicBtnUI();
      showToast('Autoplay diblokir — ketuk layar untuk mengaktifkan suara');

      // try to start muted playback (so audio context is primed) — optional
      try {
        bgAudio.muted = true;
        await bgAudio.play();
        isBgPlaying = true;
        ...
        // couldn't autoplay even muted — that's fine, we'll wait for interaction
        isBgPlaying = false;
      }
      ...
      const unlock = async () => {
        try {
          ...
          showToast('Silakan ketuk layar untuk memutar musik');
        }
